...
export class MockSuiClient implements SuiClientI {
	...
	mintNbtcBatch = jest.fn(async (mintArgs: MintBatchArg[]): Promise<SuiTxDigest> => {
		return `fake-sui-tx-digest-batch-${mintArgs.length}`;
	});

	tryMintNbtcBatch = jest.fn(async (mintArgs: MintBatchArg[]): Promise<SuiTxDigest | null> => {
